...
/**
 * Validator for ensuring that DiscogsQueryDTO is properly validated
 * when the artist is one of the "Various Artists". This validator
 * checks if the provided DTO meets the necessary criteria based on
 * the presence of certain fields when the artist is identified as
 * "Various Artists".
 */
@Slf4j
@Component
public class VariousArtistsValidator
     ...
     * "Various Artists". If the artist in the DTO matches one of the
     * predefined "Various Artists" values, it checks whether either
     * the track or album fields are provided.
     *
     * @param discogsQueryDTO the DiscogsQueryDTO object to be
     *                        validated
     * @param context         the context in which the constraint
     *                        is evaluated
     * @return true if the DTO is valid or if it does not match the criteria
     * for "Various Artists"; false otherwise
     */
    @Override
    public boolean isValid(final DiscogsQueryDTO discogsQueryDTO,
        ...
        if (isVariousArtist) {
            ...
            log.error("Invalid DiscogsQueryDTO: 'Various Artists' must be supplied with either a track or an album.");
            return false;
        }
