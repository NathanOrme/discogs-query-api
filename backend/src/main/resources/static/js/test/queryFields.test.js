...
import {
  ...
  discogFormats,
  discogCountries,
} from "./../modules/discogsData.js"; // Adjust the path if needed
...
describe("createQueryFields", () => {
  ...
  test("should create a new query field with correct elements and content", () => {
    ...
    expect(formatSelect.innerHTML).toContain(
      discogFormats
        .map((f) => `<option value="${f.value}">${f.text}</option>`)
        .join(""),
    );
    expect(countrySelect.innerHTML).toContain(
      discogCountries
        .map((c) => `<option value="${c.value}">${c.text}</option>`)
        .join(""),
    );
    expect(typesSelect.innerHTML).toContain(
      discogsTypes
  ...
  test("should show alert if both artist and barcode are empty and submit is clicked", () => {
    ...
    expect(alertMock).toHaveBeenCalledWith(
      "Please provide either an artist name or a barcode.",
    );
