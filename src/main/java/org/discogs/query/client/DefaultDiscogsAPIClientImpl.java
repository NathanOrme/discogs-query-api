...
public class DefaultDiscogsAPIClientImpl implements DiscogsAPIClient {
    ...
    /**
     * Log message used when a cache miss occurs for a search URL.
     */
    ...
    public DiscogsResult getResultsForQuery(final String searchUrl) {
        ...
        return executeWithRateLimitAndRetry(() -> httpRequestService
                        .executeRequest(searchUrl, DiscogsResult.class),
    ...
    public String getStringResultForQuery(final String searchUrl) {
        log.info(CACHE_MISS_FOR_SEARCH_URL, searchUrl);
        return executeWithRateLimitAndRetry(() -> httpRequestService
                        .executeRequest(searchUrl, String.class),
                "Discogs Search API Request");
    }
    ...
    @Override
    public DiscogsMarketplaceResult getMarketplaceResultForQuery(
            final String url) {
    ...
    /**
     * Retrieves a {@link DiscogsRelease} object for a given URL.
     * This method is cached using Spring's caching abstraction with Caffeine.
     *
     * @param url the URL pointing to the item on the Discogs Marketplace.
     ...
     * action and retries the action in case of failure.
     *
    ...
    private <T> T executeWithRateLimitAndRetry(final Callable<T> action,
        ...
        try {
            ...
            if (isDiscogsMarketplaceResult(action)) {
    ...
    private static <T> boolean isDiscogsMarketplaceResult(
            final Callable<T> action) {
        return DiscogsMarketplaceResult.class
