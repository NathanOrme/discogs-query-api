 ...
 *
 * <p>This class is responsible for deserializing JSON representation of {@link DiscogCountries}
 * enum values during the deserialization process. It converts a JSON string into the corresponding
 * {@link DiscogCountries} enum constant.
 *
...
public class DiscogsCountryDeserializer extends JsonDeserializer<DiscogCountries> {
     ...
     * attempts to match it with one of the {@link DiscogCountries} enum constants using the {@link
     * DiscogCountries#valueOf(String)} method. If the JSON string is null, empty, or does not match
     * any known enum constants, the method will return {@code null}. This behavior can be adjusted
     * based on application needs, for example by returning a default value like {@link
     * DiscogCountries#UNKNOWN}.
     *
     * @param jp   the JSON parser used to read the JSON content
     * @param ctxt the deserialization context
     * @return the corresponding {@link DiscogCountries} enum constant, or {@code null} if no match is
     * found
