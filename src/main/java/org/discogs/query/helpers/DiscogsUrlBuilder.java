...
public class DiscogsUrlBuilder {
    ...
    public String buildReleaseUrl(final DiscogsEntry discogsEntry) {
        ...
        String releaseUrl = getUrlString(
    ...
    public String buildCollectionSearchUrl(final String username, final String releaseId) {
        ...
        String fullUrl = getUrlString(UriComponentsBuilder.fromHttpUrl(discogsBaseUrl.concat(collectionsUrl)));
    ...
    /**
     * Adds query parameters to the URI builder based on the DiscogsQueryDTO.
     * If a barcode is supplied, only the barcode parameter is used.
     *
    ...
    private boolean handleBarcodeParam(final UriComponentsBuilder uriBuilder, final DiscogsQueryDTO discogsQueryDTO) {
        if (stringHelper.isNotNullOrBlank(discogsQueryDTO.barcode())) {
            ...
            uriBuilderHelper.addIfNotNullOrBlank(uriBuilder, DiscogQueryParams.BARCODE.getQueryType(),
    ...
    /**
     * Handles adding other query parameters (artist, album, track, format, country, type) to the URI builder.
     *
     * @param uriBuilder      the URI builder
     * @param discogsQueryDTO the DTO containing query parameters
     */
    private void handleOtherParams(final UriComponentsBuilder uriBuilder, final DiscogsQueryDTO discogsQueryDTO) {
        uriBuilderHelper.addIfNotNullOrBlank(uriBuilder, DiscogQueryParams.ARTIST.getQueryType(),
                discogsQueryDTO.artist());
        uriBuilderHelper.addIfNotNullOrBlank(uriBuilder, DiscogQueryParams.ALBUM.getQueryType(),
                discogsQueryDTO.album());
        uriBuilderHelper.addIfNotNullOrBlank(uriBuilder, DiscogQueryParams.TRACK.getQueryType(),
                discogsQueryDTO.track());
        uriBuilderHelper.addIfNotNullOrBlank(uriBuilder, DiscogQueryParams.FORMAT.getQueryType(),
                discogsQueryDTO.format());

        if (discogsQueryDTO.country() != null) {
            String countryName = discogsQueryDTO.country().getCountryName();
            uriBuilderHelper.addIfNotNullOrBlank(uriBuilder, DiscogQueryParams.COUNTRY.getQueryType(), countryName);
        }
