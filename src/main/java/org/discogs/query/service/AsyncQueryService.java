...
/**
 * Optimized async service that uses dedicated thread pools instead of parallelStream() to provide
 * better resource control and performance.
 */
@Slf4j
@Service
...
public class AsyncQueryService {
  ...
  /**
   * Creates optimized asynchronous tasks for Discogs API queries using dedicated thread pools
        ...
        .map(
            query ->
                CompletableFuture.supplyAsync(
                        () -> {
                          LogHelper.debug(
                              () -> "Processing query on thread: {} - Query: {}",
                    ...
                    .whenComplete(
                        (result, throwable) -> {
                          if (throwable != null) {
                            LogHelper.error(
                                () -> "Query failed on thread: {} - Error: {}",
                                Thread.currentThread().getName(),
                                throwable.getMessage());
                          } else {
                            LogHelper.debug(
                                () -> "Query completed successfully on thread: {} - Results: {}",
                                Thread.currentThread().getName(),
                                result.results().size());
                          }
                        }))
