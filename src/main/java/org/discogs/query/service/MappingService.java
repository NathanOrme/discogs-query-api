...
public class MappingService {
    ...
    DiscogsMapResultDTO convertEntriesToMapByTitle(final DiscogsResultDTO discogsResultDTO) {
        ...
        // Use the teeing collector (Java 12+) to group entries by title and determine the cheapest entry in one pass.
        return entries.stream().collect(Collectors.teeing(
                Collectors.groupingBy(DiscogsEntryDTO::title),
                Collectors.minBy(Comparator.comparing(DiscogsEntryDTO::lowestPrice)),
                (groupMap, minOpt) -> new DiscogsMapResultDTO(discogsResultDTO.searchQuery(), groupMap,
                        minOpt.orElse(null))
        ));
    }
