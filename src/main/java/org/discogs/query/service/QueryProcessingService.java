...
public class QueryProcessingService {
        ...
        return results.stream()
                .map(discogsResultDTO -> {
                    ...
                    List<DiscogsEntryDTO> filteredEntries =
                            discogsResultDTO.results().parallelStream().filter(this::isUKMarketplaceEntry).toList();
    ...
    /**
     * Retrieves the result of a {@link CompletableFuture} with a specified timeout. If the future does not complete
     * within the timeout, the task is canceled.
     *
