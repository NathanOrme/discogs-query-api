...
class UriBuilderHelperTest {
    ...
    void testAddIfNotNullOrBlank_withNonNullNonBlankValue() {
        uriBuilderHelper.addIfNotNullOrBlank(uriComponentsBuilder, "name", "JohnDoe");
        String resultUri = uriComponentsBuilder.toUriString();
        assertEquals("?name=JohnDoe", resultUri, "Expected query parameter 'name=JohnDoe' to be added");
    }
    ...
    void testAddIfNotNullOrBlank_withBlankValue() {
        ...
        String resultUri = uriComponentsBuilder.toUriString();
        assertEquals("", resultUri, "Expected no query parameter to be added for blank value");
    }

    @Test
    void testAddIfNotNullOrBlank_withNullValue() {
        uriBuilderHelper.addIfNotNullOrBlank(uriComponentsBuilder, "name", null);
        String resultUri = uriComponentsBuilder.toUriString();
        assertEquals("", resultUri, "Expected no query parameter to be added for null value");
    }
    ...
    void testAddIfNotNull_withNonNullValue() {
        ...
        String resultUri = uriComponentsBuilder.toUriString();
        assertEquals("?age=25", resultUri, "Expected query parameter 'age=25' to be added");
    }
